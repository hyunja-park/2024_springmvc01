...
public class SnSLoginController {
	...
	public ModelAndView kakaologin(HttpServletRequest request) {
		...
		try {
			...
			sb.append("&client_id=fcb95810d0b0d2ccf6dc143279ce8983");
			sb.append("&redirect_uri=http://localhost:8080/kakaologin");
			sb.append("&code=" + code);
			sb.append("&client_secret=GMGBFA4eYgo8iToAdclc6FTM2e1zNT9L");
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				System.out.println(kavo.getTokon_type());
	...
	public ModelAndView naverlogin(HttpServletRequest request) {
		...
		// 2. 토큰받기(인증코드 필요)
		// String reqURL = "https://kauth.kakao.com/oauth/token"; 카카오방식
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("grant_type=authorization_code");
			sb.append("&client_id=rA54ouV8itEpmcqU3Tx6");
			sb.append("&client_secret=grouJAz_wU");
			sb.append("&code=" + code);
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				// 해당정보를 Ajax 컨트롤러를 사용하기 위해서 세션에 저장
	...
	public ModelAndView daumAddrOK(AddrVO addrvo, HttpSession session) {
		...
		// 로그인 성공한 상태에서 session 안에 있는 사용자 정보 중 m_id나 사용자 테이블의 pk를 호출한다.

...
public class FileController {
	...
	public ModelAndView fileUp(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			// 실전에서는 프로젝트가 아닌다른 곳에 저장 할 수도 있다. (AWS의 S3의 서버나 특정위치)
			String path = request.getSession().getServletContext().getRealPath("/resources/upload");
			
			
			// cos 라이브러리를 사용하면 request를 사용하는것이 아니다.
			MultipartRequest mr =
					new MultipartRequest(
							request,        // request 대신하기 위해서  request를 받는다.
							path, 			// 저장위치											
							500*1024*1024,  // 한번에 업로드할 수 있는 최대크기 // 500MB(2024(KB)*1024(MB) *1024(GB)*1024(TB))
							"utf-8", 		// 한글처리
	...
	public ModelAndView fileUp2(
		...
		try {
			...
			// 아니면 UUID를 생성해서 붙인다.(범용 고유 식별자를 의미하며 중복이되지 않는 유일한 값을 구성하고자 할때 주로 사용)
			UUID uuid = UUID.randomUUID();
			String fname = uuid.toString()+"-"+f_name.getOriginalFilename();
